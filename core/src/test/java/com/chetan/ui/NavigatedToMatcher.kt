 ...
 */
fun navigatedTo(navigateTo: NavigateTo): Matcher<Navigator?> = NavigatedToMatcher(Event(navigateTo))

/**
 * Returns matcher that checks that [Navigator] has navigated to given direction.
 */
fun navigatedTo(direction: NavDirections): Matcher<Navigator?> = NavigatedToMatcher(Event(
    NavigateTo.Action(
        direction
    )
))
